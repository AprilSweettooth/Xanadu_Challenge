...
def state_purity(angle, phase, circuit_param, noise_param):
    ...
    def noisy_circuit(angle, phase, circuit_param, noise_param):
        ...
        qml.T(wires=0)
        noise(noise_param)
        # qml.transforms.insert(circuit(circuit_param), noise(noise_param), all)
    ...
    mat = noisy_circuit(angle, phase, circuit_param, noise_param)
    print(mat)
    print(np.matmul(mat,mat))
    print(np.trace(np.matmul(mat,mat)))
    return np.trace(np.matmul(mat,mat))
    # Return the purity in terms of the calculated expectation values.
