...
def qnode_ansatzes(d, scale_factors):
    ...
    qnodes_noisy = qml.QNode(circuit, dev_noisy)
    qnodes_mitigated = [qml.transforms.fold_global(qnodes_noisy, s) for s in scale_factors]
...
def extrapolation(d, scale_factors, plot=False):
    ...
    # Put your code here #
    zne_energy = qml.transforms.poly_extrapolate(scale_factors, mitigated_energies, 2)
    return np.array([ideal_energy, zne_energy]).tolist()

print(extrapolation(0.6614, [1,2,3]))
