...
def average_fidelity(gate_list, wire_list, noise_param, over_rot):
    ...
    def custom_cnot(wires, param=[over_rot,noise_param]):
        # Put your code here #
        # Return your custom CNOT gate as implemented by a superconducting qubit as an array, including noise
        # See documentation for more details!
        return [
        qml.RZ(param[0]-np.pi/2, wires=wires[0]),
        qml.RX(param[0]+np.pi/2, wires=wires[1]),
        qml.RZ(param[0]+np.pi/2, wires=wires[1]),
        qml.ISWAP(wires=[wires[0],wires[1]]),
        qml.RX(param[0]+np.pi/2, wires=wires[0]),
        qml.ISWAP(wires=[wires[0],wires[1]]),
        qml.RZ(param[0]+np.pi/2, wires=wires[1]),
        qml.DepolarizingChannel(param[1], wires=wires[0]),
        qml.DepolarizingChannel(param[1], wires=wires[1])
        ]
    # param = [noise_param, over_rot]
    custom_decomps = {qml.CNOT : custom_cnot}
    # Write here the custom decomposition of the CNOT gate for your device.
